import { Developer, RankingWeights } from '../types/developer'
 ...
 */
function normalizeDevRank(score: number): number {
 ...
 */
function normalizeActivity(commits: number): number {
  if (commits === 0) return 0
  if (commits >= 100) return 100
 ...
 */
function normalizeFollowers(followers: number): number {
...
export function calculateMatchScore(
  ...
  if (requirements.requiredLanguages && developer.top_languages) {
    const matchedLanguages = developer.top_languages.filter(lang =>
      requirements.requiredLanguages!.some(
        req => req.toLowerCase() === lang.toLowerCase()
      )
    )
    const languageScore =
      (matchedLanguages.length / requirements.requiredLanguages.length) * 30
    score += languageScore
  }
